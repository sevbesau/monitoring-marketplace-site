    ...
    <div>
      ...
      <div class="lg:hidden">
        ...
        <div v-if="showMenu" class=" absolute mt-8 right-0 left-0 flex flex-col shadow-lg bg-black z-50">
          ...
          <router-link to="/" class="border-t border-b px-8 py-4">Explore</router-link>
      ...
      <div class="hidden lg:flex items-center">
        <router-link class="mx-8 text-lg" to="/">Explore</router-link> 
        <router-link class="mx-8 text-lg opacity-60" to="/build">Build</router-link> 
        <div v-if="user && user.firstname" class="flex items-center">
          <p>Welcome {{user.firstname}}!</p>
          <p class="border mx-3 text-center rounded opacity-60 p-2 cursor-pointer" @click="logout">LOGOUT</p>
        </div>
